...
### 🔹 전략 패턴 핵심 키워드 정리 (짧게 추가용)

**📘 목적**

* 알고리즘(행동)을 **캡슐화**하고, **런타임에 교체 가능**하게 함

**📂 구조**

* `Strategy` → 알고리즘 인터페이스
* `ConcreteStrategy` → 실제 알고리즘 구현
* `Context` → 전략을 보유(has-a), 실행은 위임(delegate)

**⚙️ 관계**

* **is-a** → 상속, 재사용 중심
* **has-a** → 구성(Composition), 유연성 중심 ← ✅ 전략 패턴 구조

**💡 특징**

* OCP(개방-폐쇄 원칙) 충족
* 상속보다 **구성(위임)** 사용
* 변하는 것(전략)과 변하지 않는 것(Context)을 분리

**🧠 실제 예시**

* `Comparator` (Java Collections)
* `AuthenticationProvider` (Spring Security)
* 결제 방식 선택 (Card / Crypto / Point 등)
