...
| -------------- | ----------------------- | ------------------------ |
| **생명주기 관리**    | 어려움 (프로그램 흐름 전체에 퍼져 있음) | 클래스 내부에서 완전 통제           |
| **네임스페이스 혼잡**  | 매우 높음 (충돌 위험 존재)        | 캡슐화되어 충돌 위험 낮음           |
| **인스턴스 개수 강제** | 불가 (누구나 새로운 객체 생성 가능)   | 1개로 강제됨 (생성 제어)          |
| **객체지향적 구조**   | 낮음 (절차지향적 느낌)           | 높음 (정보 은닉 + 캡슐화)         |
| **예상 가능성**     | 낮음 (아무 곳에서나 값 변경 가능)    | 높음 (접근 경로 통일됨)           |
| **테스트 용이성**    | 매우 낮음                   | DI/Mock으로 대체 가능 (상대적 우위) |
